...
token = "7433181709:AAEf51Mex8xmeJp57ZczT3EgldCbq9VmSEY" #توكنك
...
async def claimer(client,username):
    result = await client(functions.channels.CreateChannelRequest(title=f'Claim [ {username} ]',about=f'Source - @bxiii ',megagroup=False))
    try:
...
def reset(msg):
    if db.exists('session') is False:
     bot.send_message(chat_id, text="اهلا بك عزيزي\nبوت متخصص للصيد \n– – – – –\nمطوره : @bxiii . ", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="اضف حساب",callback_data="add")]]))
     return 
    bot.send_message(chat_id, text="اهلا بك عزيزي\nبوت متخصص للصيد \n– – – – –\nمطوره : @bxiii ", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="بدء الفحص", callback_data="start")],[InlineKeyboardButton(text="من ملف", callback_data="combo"),InlineKeyboardButton(text="تثبيت", callback_data="us1")],[InlineKeyboardButton(text="اختيار نوع", callback_data="type"),]]))
